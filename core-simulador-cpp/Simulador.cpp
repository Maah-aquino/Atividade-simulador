...
Personagem* Simulador::proximoPersonagem(vector<Personagem*> equipe)
{
 ...
 // Define a semente para a função rand() com base no tempo atual
    ...
    // Gera um número aleatório e usa o operador módulo para obter um índice válido na lista
    int indiceAleatorio = std::rand() % tamanho; // define o intervalo entre 0 e tamanho -1

    // Verifica se o personagem no índice aleatório tem vida
    ...
    // Se o personagem selecionado não tiver vida, inicia um loop para encontrar um personagem com vida
    for (auto &personagem : equipe)
    {
        // Verifica se o personagem atual no loop tem vida
 ...
 int contador = 0;
    ...
    return nullptr; // Caso não haja personagens com vida
}
